...
from mpcomp.views import get_resume_data, handle_uploaded_file, jobseeker_login_required
...
def my_home(request):
    ...
    user_certifications = Certification.objects.filter(user=user).order_by(
    ...
    try:
        ...
        with transaction.atomic():
            ...
            if mobile:
                ...
                existing_user = (
                    User.objects.filter(mobile=mobile).exclude(id=user.id).first()
                )
                if existing_user:
                    return JsonResponse(
                        {
        ...
        return JsonResponse(
            {"success": True, "message": "Personal information updated successfully!"}
        )
...
def edit_project_modal(request, project_id):
    ...
    if request.method == "GET":
        ...
        project_data = {
            ...
            "from_date": project.from_date.strftime("%Y-%m-%d")
        ...
        skills_data = [
            ...
            for skill in Skill.objects.filter(status="Active").order_by("name")
        ]
        cities_data = [
        ...
        if form.is_valid():
            ...
            if (
                request.user.project.filter(name=request.POST.get("name"))
            ...
            return JsonResponse(
                {
                    ...
                    "project": {
                        ...
                        "name": project.name,
                        "description": project.description,
                        "from_date": project.from_date.strftime("%B %Y")
                        if project.from_date
                        else "",
                        "to_date": project.to_date.strftime("%B %Y")
                        if project.to_date
                        else "Present",
                        "role": project.role or "",
                        "size": project.size or "",
...
def edit_language_modal(request, language_id):
    ...
    try:
        ...
        if user_language not in request.user.language.all():
            return JsonResponse(
                {"success": False, "error": "Language not found or access denied"}
            )
        ...
        if request.method == "GET":
            ...
            if language_obj != user_language.language:
                existing_language = request.user.language.filter(
                    ...
                    return JsonResponse(
                        {
                            "success": False,
                            "errors": {
                                "language": "You already have this language in your profile"
                            },
                        }
                    )
            ...
            return JsonResponse(
                {"success": True, "message": "Language updated successfully"}
            )
    ...
    try:
        ...
        with transaction.atomic():
            user_language = UserLanguage.objects.create(
                language=language_obj, read=read, write=write, speak=speak
            )
...
def edit_experience_modal(request, experience_id):
    ...
    try:
        ...
        if not request.user.employment_history.filter(id=experience_id).exists():
            return JsonResponse(
                {"success": False, "message": "Experience not found or access denied"},
                status=404,
            )
        ...
        if request.method == "GET":
            ...
            experience_data = {
                ...
                "from_date": experience.from_date.strftime("%Y-%m-%d")
                if experience.from_date
                else "",
                "to_date": experience.to_date.strftime("%Y-%m-%d")
            ...
            return JsonResponse(
                {"success": True, "data": {"experience": experience_data}}
            )
        elif request.method == "POST":
            data = json.loads(request.body)
            errors = {}
            ...
            with transaction.atomic():
                ...
                experience.to_date = (
                    to_date if not data.get("current_job", False) else None
                )
            ...
            "experiences": [
                {
                    ...
                    "from_date": exp.from_date.strftime("%Y-%m-%d")
                    if exp.from_date
                    else None,
                    "to_date": exp.to_date.strftime("%Y-%m-%d")
                    if exp.to_date
                    else None,
...
def edit_education_modal(request, education_id):
    ...
    try:
        ...
        return JsonResponse(
            {"success": False, "message": "Education not found"}, status=404
        )
    ...
    if request.method == "GET":
        ...
        education_data = {
            ...
            "institute_city": education.institute.city.id
            if education.institute.city
            else "",
            "degree_name": education.degree.degree_name.id
            if education.degree.degree_name
            else "",
            "degree_type": education.degree.degree_type,
            "specialization": education.degree.specialization,
            "from_date": education.from_date.strftime("%Y-%m-%d")
            if education.from_date
            else "",
            "to_date": education.to_date.strftime("%Y-%m-%d")
        ...
        try:
            with transaction.atomic():
                ...
                if (
                    education.institute.name != institute_name
                    or education.institute.city != institute_city
                ):
                ...
                try:
                    ...
                    return JsonResponse(
                        {"success": False, "error": "Invalid degree selected"}
                    )

                # Update degree or create new one if details changed
                if (
                    education.degree.degree_name != degree_name
                    or education.degree.degree_type != degree_type
                    or education.degree.specialization != specialization
                ):
                    degree, _created = Degree.objects.get_or_create(
                ...
                try:
                    education.from_date = datetime.strptime(
                        from_date, "%Y-%m-%d"
                    ).date()
                    if to_date and not current_education:
                        education.to_date = datetime.strptime(
                            to_date, "%Y-%m-%d"
                        ).date()
                ...
                return JsonResponse(
                    {"success": True, "message": "Education updated successfully"}
                )
...
def add_education_modal(request):
    ...
    try:
        with transaction.atomic():
            ...
            institute, _created = EducationInstitue.objects.get_or_create(
                name=institute_name, city=institute_city, defaults={"address": ""}
            )
            ...
            return JsonResponse(
                {"success": True, "message": "Education added successfully"}
            )
    ...
    if request.method == "GET":
        ...
        try:
            ...
            try:
                ...
                return JsonResponse(
                    {"success": False, "message": "Selected skill does not exist."}
                )
            ...
            with transaction.atomic():
                ...
                if last_used:
                    try:
                        technical_skill.last_used = datetime.strptime(
                            last_used, "%Y-%m-%d"
                        ).date()
            ...
            return JsonResponse(
                {"success": True, "message": "Technical skill added successfully!"}
            )
...
def edit_skill_modal(request, skill_id):
    ...
    try:
        ...
        if technical_skill not in request.user.skills.all():
            return JsonResponse(
                {"success": False, "message": "Skill not found or access denied."},
                status=404,
            )
        ...
        if request.method == "GET":
            ...
            skill_data = {
                ...
                "last_used": technical_skill.last_used.strftime("%Y-%m-%d")
            ...
            try:
                ...
                if (
                    request.user.skills.filter(skill=skill)
                    .exclude(id=technical_skill.id)
                    .exists()
                ):
                    return JsonResponse(
                        {
...
def delete_skill_modal(request, skill_id):
    ...
    try:
        ...
        return JsonResponse(
            {"success": True, "message": "Technical skill deleted successfully!"}
        )

    except Exception:
        return JsonResponse(
            {"success": False, "message": "An error occurred while deleting the skill."}
        )
    ...
    if request.method == "GET":
        ...
        try:
            ...
            functional_areas_data = [
                ...
                for area in FunctionalArea.objects.filter(status="Active").order_by(
                    "name"
                )
            ]
            industries_data = [
                {"id": industry.id, "name": industry.name}
                for industry in Industry.objects.filter(status="Active").order_by(
                    "name"
                )
            ]
            ...
            return JsonResponse(
                {"success": False, "error": "Failed to load job preferences data."}
            )
        ...
        try:
            ...
            with transaction.atomic():
                ...
                if functional_area_ids:
                    functional_areas = FunctionalArea.objects.filter(
                        id__in=functional_area_ids, status="Active"
                    )
                ...
                if industry_ids:
                    industries = Industry.objects.filter(
                        id__in=industry_ids, status="Active"
                    )
            ...
            return JsonResponse(
                {"success": True, "message": "Job preferences updated successfully!"}
            )
...
def add_certification_modal(request):
    ...
    if request.method == "GET":
        ...
        try:
            ...
            if issued_date:
                certification.issued_date = datetime.strptime(
                    issued_date, "%Y-%m-%d"
                ).date()

            expiry_date = data.get("expiry_date")
            if expiry_date and not certification.does_not_expire:
                certification.expiry_date = datetime.strptime(
                    expiry_date, "%Y-%m-%d"
                ).date()
            ...
            if (
                ...
                and certification.issued_date > certification.expiry_date
            ):
                return JsonResponse(
                    {
                        "success": False,
            ...
            return JsonResponse(
                {
                    ...
                    "certification": {
                        ...
                        "issued_date": certification.issued_date.strftime("%Y-%m-%d")
                        if certification.issued_date
                        else "",
                        "expiry_date": certification.expiry_date.strftime("%Y-%m-%d")
...
def edit_certification_modal(request, certification_id):
    ...
    try:
        certification = Certification.objects.get(
            id=certification_id, user=request.user
        )
    except Certification.DoesNotExist:
        return JsonResponse(
            {"success": False, "message": "Certification not found"}, status=404
        )
...
def upload_resume_modal(request):
    ...
    try:
        if "resume" not in request.FILES:
            return JsonResponse({"success": False, "error": "No resume file provided"})
        ...
        if file_size_kb > 1000 or file_size_kb <= 0:
            return JsonResponse(
                {"success": False, "error": "File size must be between 1KB and 1000KB"}
            )
        ...
        try:
            ...
            try:
                ...
                _email, mobile, text = get_resume_data(resume_file)
            ...
            return JsonResponse(
                {
                    "success": False,
                    "error": f"Failed to upload resume: {upload_error!s}",
                }
            )
        ...
        return JsonResponse(
            {
                "success": False,
                "error": f"An error occurred while uploading your resume: {e!s}",
            }
        )
...
def delete_resume_modal(request):
    ...
    try:
        if not request.user.resume:
            return JsonResponse(
                {"success": False, "error": "No resume found to delete"}
            )
        ...
        return JsonResponse(
            {
                "success": False,
                "error": f"An error occurred while deleting your resume: {e!s}",
            }
        )
...
def get_resume_info(request):
    ...
    try:
        if not request.user.resume:
            return JsonResponse({"success": True, "has_resume": False, "data": None})
        ...
        try:
            ...
            if not request.user.resume or not request.user.resume.name:
                return JsonResponse(
                    {"success": True, "has_resume": False, "data": None}
                )
            ...
            return JsonResponse(
                {
                    "success": False,
                    "error": f"Resume file not accessible. This may be due to storage configuration. Error: {file_error!s}",
                }
            )
        ...
        return JsonResponse(
            {
                ...
                "data": {
                    ...
                    "resume_url": resume_url,
                    "upload_date": request.user.profile_updated.strftime("%B %d, %Y")
        ...
        return JsonResponse(
            {
                "success": False,
                "error": f"An error occurred while getting resume information: {e!s}",
            }
        )
    ...
    try:
        ...
        try:
            ...
            return JsonResponse(
                {
                    "success": False,
                    "error": f"Failed to update resume: {upload_error!s}",
                }
            )
        ...
        return JsonResponse(
            {
                "success": False,
                "error": f"An error occurred while updating your resume: {e!s}",
            }
        )
    ...
    try:
        ...
        with transaction.atomic():
            ...
            if current_city_id:
                try:
                    current_city = City.objects.get(
                        id=current_city_id, status="Enabled"
                    )
            ...
            response_data = {
                ...
                "current_city": f"{user.current_city.name}, {user.current_city.state.name}"
        ...
        return JsonResponse(
            {
                "success": False,
                "error": f"An error occurred while updating your profile: {e!s}",
            }
        )
