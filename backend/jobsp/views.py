...
from pjob.views import add_other_location_to_user, save_codes_and_send_mail
...
def forgot_password(request):
    ...
    if request.method == "POST":
        try:
            ...
            if form_valid.is_valid():
                ...
                if user:
                    ...
                    try:
                        ...
                        logger.info(
                            f"Password reset email task queued successfully for: {mto}"
                        )
                    except Exception:
                        ...
                        msg = EmailMessage(
                            subject, rendered, settings.DEFAULT_FROM_EMAIL, [mto]
                        )
            ...
            logger.error(f"Exception during password reset: {e!s}")
            data = {
                "error": True,
...
def set_password(request, user_id, passwd_reset_token):
    ...
    user = User.objects.filter(
        id=user_id, password_reset_token=passwd_reset_token
    ).first()
        ...
        if token_age > timedelta(hours=24):  # Token expires after 24 hours
            ...
            return render(
                ...
                template,
                {
                    ...
                    "reason": "Please request a new password reset link",
                },
                status=410,
            )
