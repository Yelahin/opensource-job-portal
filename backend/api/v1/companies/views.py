...
from peeldb.models import City, Company, Industry, JobPost
    ...
    def get_queryset(self):
        ...
        if location:
            ...
            try:
                ...
                company_ids = (
                    JobPost.objects.filter(location=city, status="Live")
        ...
        if industry:
            ...
            try:
                ...
                company_ids = (
                    JobPost.objects.filter(industry=ind, status="Live")
                    .values_list("company_id", flat=True)
                    .distinct()
                )
                queryset = queryset.filter(id__in=company_ids)
...
def company_filter_options(request):
    ...
    company_types = (
        active_companies.values("company_type")
    ...
    sizes = (
        active_companies.values("size").annotate(count=Count("id")).order_by("-count")
    )

    # Get locations from active job posts
    locations = (
        City.objects.filter(
            locations__status="Live", locations__company__is_active=True
        )
        .annotate(count=Count("locations", distinct=True))
        .order_by("-count")[:20]
    )

    location_data = [
        {"label": f"{loc.name}", "value": loc.slug, "count": loc.count}
    ...
    industries = (
        Industry.objects.filter(
            jobpost__status="Live", jobpost__company__is_active=True
        )
        .annotate(count=Count("jobpost", distinct=True))
        .order_by("-count")[:20]
    )

    industry_data = [
        {"label": ind.name, "value": ind.slug, "count": ind.count} for ind in industries
    ]
