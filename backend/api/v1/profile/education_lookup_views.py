...
from peeldb.models import Degree, EducationInstitue, Qualification
...
class DegreeViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    @extend_schema(
        ...
        responses={
            200: OpenApiResponse(
                response=DegreeSerializer(many=True), description="List of degrees"
            )
        },
        tags=["Education Lookups"],
    )
    def list(self, request):
        ...
        queryset = queryset.select_related("degree_name").order_by(
            "degree_name__name", "specialization"
        )
        serializer = self.get_serializer(queryset, many=True)
        return Response(serializer.data, status=status.HTTP_200_OK)
