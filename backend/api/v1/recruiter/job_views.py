...
from .job_serializers import (
    ...
    RecruiterApplicantStatusRequestSerializer,
    RecruiterApplicantStatusResponseSerializer,
    RecruiterDashboardResponseSerializer,
    RecruiterJobApplicantsResponseSerializer,
    RecruiterJobCreateSerializer,
    RecruiterJobDeleteConflictSerializer,
...
@extend_schema(
    ...
    parameters=[
        OpenApiParameter(
            "status",
            OpenApiTypes.STR,
            description="Filter by status (Live, Draft, Expired, etc.)",
        ),
        OpenApiParameter(
            "search", OpenApiTypes.STR, description="Search by title or company name"
        ),
        OpenApiParameter(
            "ordering",
            OpenApiTypes.STR,
            description="Order by field (created_on, -created_on, title, -applicants)",
        ),
    ...
    # Paginated when a page is requested, otherwise a bare list; documented as
    # the list form, which is what the SvelteKit client reads.
    responses={200: RecruiterJobListSerializer(many=True)},
)
@api_view(["GET"])
...
def list_jobs(request):
    ...
    if ordering == "applicants" or ordering == "-applicants":
        queryset = queryset.order_by(
            "applicants_count" if ordering == "applicants" else "-applicants_count"
        )
    else:
    ...
    if page is not None:
        serializer = RecruiterJobListSerializer(
...
@extend_schema(
    ...
    responses={
        200: RecruiterJobDetailSerializer,
...
@extend_schema(
    ...
    responses={
        201: RecruiterJobMutationResponseSerializer,
...
def create_job(request):
    ...
    if serializer.is_valid():
        ...
        response_serializer = RecruiterJobDetailSerializer(
...
@extend_schema(
    ...
    responses={
        ...
        # Two different 400 bodies: refusing to delete a published job (plain
        # error) and refusing one that has applicants (error + count).
        400: RecruiterJobDeleteConflictSerializer,
        404: ErrorResponseSerializer,
    },
)
@api_view(["DELETE"])
...
def delete_job(request, job_id):
    ...
    return Response(
        {"success": True, "message": f"Job '{job_title}' deleted successfully"}
    )
...
def publish_job(request, job_id):
    ...
    if job.status == "Live":
        return Response(
            {"error": "Job is already live"}, status=status.HTTP_400_BAD_REQUEST
        )
...
def close_job(request, job_id):
    ...
    if job.status == "Disabled":
        return Response(
            {"error": "Job is already closed"}, status=status.HTTP_400_BAD_REQUEST
        )
    ...
    serializer = RecruiterJobDetailSerializer(job, context={"request": request})
    return Response(
        {"success": True, "job": serializer.data, "message": "Job closed successfully"}
    )
...
@extend_schema(
    ...
    parameters=[
        ...
        OpenApiParameter(
            "ordering",
            OpenApiTypes.STR,
            description="Order by field (applied_on, -applied_on)",
        ),
    ],
    responses={
        200: RecruiterJobApplicantsResponseSerializer,
        404: ErrorResponseSerializer,
    },
)
@api_view(["GET"])
...
@extend_schema(
    ...
    parameters=[
        OpenApiParameter(
            "period",
            OpenApiTypes.STR,
            description="Time period for trend calculation: 7d, 30d, 90d (default: 30d)",
        ),
    ],
    responses={200: RecruiterDashboardResponseSerializer},
)
@api_view(["GET"])
@permission_classes([IsAuthenticated])
def get_dashboard_stats(request):
    ...
    new_applicants = all_applicants.filter(applied_on__gte=start_date).count()

    # NEW: Applications in previous period (for trend)
    prev_applicants = all_applicants.filter(
        applied_on__gte=prev_start_date, applied_on__lt=start_date
    ).count()
    ...
    avg_applications_per_job = total_applicants / live_jobs if live_jobs > 0 else 0
    ...
    for job in recent_jobs:
        job_data = RecruiterJobListSerializer(job, context={"request": request}).data
...
@extend_schema(
    ...
    parameters=[
        OpenApiParameter(
            "country_id", OpenApiTypes.INT, description="Country ID to filter states"
        ),
        OpenApiParameter(
            "state_id", OpenApiTypes.INT, description="State ID to filter cities"
        ),
        OpenApiParameter(
            "search", OpenApiTypes.STR, description="Search term for filtering"
        ),
    ],
    responses={200: RecruiterJobFormMetadataSerializer},
)
@api_view(["GET"])
@permission_classes([IsAuthenticated])
...
def get_applicant_detail(request, job_id, applicant_id):
    ...
    if not applicant_user:
        return Response(
            {"error": "Applicant user not found"}, status=status.HTTP_404_NOT_FOUND
        )

    serializer = ApplicantDetailSerializer(
        applicant_user, context={"request": request, "job_id": job_id}
    )
...
@extend_schema(
    ...
    request=RecruiterApplicantStatusRequestSerializer,
    responses={
        200: RecruiterApplicantStatusResponseSerializer,
        400: ErrorResponseSerializer,
        404: ErrorResponseSerializer,
    },
)
