...
from peeldb.models import TeamInvitation, User
...
from .serializers import (
    ...
    CompanyProfileUpdateResponseSerializer,
    CompanyUpdateSerializer,
    SendInvitationSerializer,
    TeamInvitationListResponseSerializer,
    TeamInvitationMutationResponseSerializer,
...
def list_team_members(request):
    ...
    members = User.objects.filter(company=user.company, user_type="EM").order_by(
        "-is_admin", "first_name"
    )
...
@extend_schema(
    ...
    responses={
        201: TeamInvitationMutationResponseSerializer,
...
def invite_team_member(request):
    ...
    serializer = SendInvitationSerializer(
        data=request.data, context={"company": user.company, "user": user}
    )
...
@extend_schema(
    ...
    responses={
        200: TeamInvitationListResponseSerializer,
        403: ErrorResponseSerializer,
    },
)
@api_view(["GET"])
...
@extend_schema(
    ...
    request=None,
    responses={
        200: TeamInvitationMutationResponseSerializer,
        403: ErrorResponseSerializer,
        404: ErrorResponseSerializer,
    },
)
...
def resend_invitation(request, invitation_id):
    ...
    try:
        invitation = TeamInvitation.objects.get(id=invitation_id, company=user.company)
...
def cancel_invitation(request, invitation_id):
    ...
    try:
        invitation = TeamInvitation.objects.get(
            id=invitation_id, company=user.company, status="pending"
        )
    except TeamInvitation.DoesNotExist:
        return Response(
            {"error": "Invitation not found"}, status=status.HTTP_404_NOT_FOUND
...
def update_team_member(request, user_id):
    ...
    serializer = UpdateTeamMemberSerializer(
        data=request.data, context={"user_to_update": member, "company": user.company}
    )
...
def remove_team_member(request, user_id):
    ...
    if member.is_admin:
        admin_count = User.objects.filter(
            company=user.company, is_admin=True, user_type="EM"
        ).count()
...
@extend_schema(
    ...
    responses={
        200: CompanyProfileUpdateResponseSerializer,
        400: VALIDATION_ERROR_RESPONSE,
        403: ErrorResponseSerializer,
    },
)
...
def update_company_profile(request):
    ...
    from .serializers import CompanyDetailSerializer, CompanyUpdateSerializer

    serializer = CompanyUpdateSerializer(
        user.company, data=request.data, partial=True, context={"request": request}
    )
    ...
    if serializer.is_valid():
        ...
        result_serializer = CompanyDetailSerializer(
