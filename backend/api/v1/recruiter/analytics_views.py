...
from peeldb.models import AppliedJobs, JobPost

from ..common.responses import ErrorResponseSerializer
...
@extend_schema(
    ...
    description="Application volume, pipeline and per-job performance across all of the recruiter's jobs.",
    parameters=[
        OpenApiParameter(
            "period",
            OpenApiTypes.STR,
            description="7d, 30d, 90d or custom (default: 30d)",
            enum=["7d", "30d", "90d", "custom"],
        ),
        OpenApiParameter(
            "start_date",
            OpenApiTypes.DATE,
            description="Window start as YYYY-MM-DD. Required when period=custom.",
        ),
        OpenApiParameter(
            "end_date",
            OpenApiTypes.DATE,
            description="Window end as YYYY-MM-DD. Required when period=custom.",
        ),
    ],
    responses={
        200: ApplicationAnalyticsResponseSerializer,
...
def get_application_analytics(request):
    ...
    applications = AppliedJobs.objects.filter(
        job_post__in=jobs, applied_on__gte=start_date, applied_on__lte=end_date
    )

    # Applications in previous period
    prev_applications = AppliedJobs.objects.filter(
        job_post__in=jobs, applied_on__gte=prev_start_date, applied_on__lt=start_date
    )
    ...
    # Applications by day
    applications_by_day = (
        applications.extra(select={"day": "DATE(applied_on)"})
        .values("day")
    ...
    peak_days_data = (
        applications.annotate(weekday=ExtractWeekDay("applied_on"))
        .values("weekday")
        .annotate(count=Count("id"))
    )
...
@extend_schema(
    tags=["Recruiter - Analytics"],
    summary="Job Application Analytics",
    description="Application volume and pipeline for a single job owned by the recruiter.",
    parameters=[
        OpenApiParameter(
            "period",
            OpenApiTypes.STR,
            description="7d, 30d or 90d (default: 30d). Unlike the account-wide "
            "endpoint, this one has no custom range.",
            enum=["7d", "30d", "90d"],
        ),
    ],
    responses={
        200: JobApplicationAnalyticsResponseSerializer,
...
def get_job_application_analytics(request, job_id):
    ...
    applications = AppliedJobs.objects.filter(job_post=job, applied_on__gte=start_date)
    ...
    if total_apps > 0:
        pipeline["conversion_rate"] = round((pipeline["hired"] / total_apps) * 100, 2)
    else:
        pipeline["conversion_rate"] = 0
