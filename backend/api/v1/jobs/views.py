...
class JobViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def saved_jobs(self, request):
        ...
        if request.method == "GET":
            # Get all saved jobs
            saved_jobs = SavedJobs.objects.filter(user=request.user).select_related(
                "job_post"
            )
            jobs = [
                saved.job_post
                for saved in saved_jobs
                if saved.job_post.status == "Live"
            ]
    ...
    def unsave_job(self, request, id=None):
        ...
        try:
            ...
            saved_job.delete()
            return Response(
                {"message": "Job unsaved successfully"}, status=status.HTTP_200_OK
            )
        except SavedJobs.DoesNotExist:
            return Response(
                {"error": "Saved job not found"}, status=status.HTTP_404_NOT_FOUND
            )
...
class JobFilterOptionsView(APIView):
    ...
    def get(self, request):
        ...
        industries = (
            Industry.objects.filter(jobpost__in=live_jobs)
            .annotate(count=Count("jobpost", filter=Q(jobpost__status="Live")))
            .order_by("-count", "name")
        ...
        education = (
            Qualification.objects.filter(jobpost__in=live_jobs)
            .annotate(count=Count("jobpost", filter=Q(jobpost__status="Live")))
            .order_by("-count", "name")
            .values("id", "name", "slug", "count")
        )
        ...
        for value, label in JOB_TYPE:
            ...
            if count > 0:
                job_types.append({"value": value, "label": label, "count": count})
