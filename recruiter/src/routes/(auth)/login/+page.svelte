<script lang="ts">
	...
	// A failed OAuth round trip comes back as ?error=… on the load, a failed
	// password submit as form.error. Only one can be present at a time.
	let error = $derived(form?.error || data.oauthError || '');
			...
			{#if data.googleAuthUrl}
				<div class="relative my-6">
					...
					<div class="relative flex justify-center">
						<span class="bg-white px-3 text-sm text-muted">or</span>
					</div>
				</div>

				<!-- A real link, so it works without JS and the browser handles the
				     cross-origin navigation to Google itself. -->
				<a
					href={data.googleAuthUrl}
					onclick={() => (googleLoading = true)}
					class="w-full inline-flex items-center justify-center gap-3 px-4 py-3 border border-border rounded-lg text-sm font-medium text-black bg-white hover:bg-surface transition-colors {googleLoading
