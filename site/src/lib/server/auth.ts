 ...
 * sees a cookie. That is what makes separate production domains work without
 * third-party cookies.
 */
 ...
 * and REFRESH_TOKEN_LIFETIME is 7 days. A shorter cookie than the token just
 * forces needless refreshes.
 */
 ...
 * Cookie options.
 *
 * `secure` follows the build: always on outside dev, because the production
 * site is HTTPS. Hardcoding it to false (as recruiter/ currently does) means
 ...
 * also returns a *new* refresh token and blacklists the one just used. The
 * caller has to persist both — keeping the old refresh cookie would blacklist
 * the session on the very next refresh.
 *
 * Returns null if the refresh token is dead, in which case the caller should
 * clear the cookies.
 */
